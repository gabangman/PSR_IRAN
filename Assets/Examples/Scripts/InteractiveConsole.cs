...
public sealed class InteractiveConsole : ConsoleBase
{
	...
	public string FeedLink = "http://apps.facebook.com/" + FB.AppId + "/?challenge_brag=" + (FB.IsLoggedIn ? FB.UserId : "guest");
	public string FeedLinkName = "I`m Just Finished!";
    public string FeedLinkCaption = "PinInRacing";
    public string FeedLinkDescription = "WIN!!!!";
	public string FeedPicture = "http://cdn.gabangmanstudio.com/Data/AssetBundle_Global/icon_128.png";
    ...
    void OnGUI()
    {
		...
		if (Button("myFacebookTest"))
		{

			SNSManager.GetFaceBookmyUserProfileURL();
			//SNSManager.GetFaceBookFriendsField();
			/*	
			FB.Feed(                                                                                                                 
			        linkCaption: "I just finished " +  " Can you beat it?",               
			        picture: "http://cdn.gabangmanstudio.com/Data/AssetBundle_Global/icon_144.png",                                            
			...
			if(mCnt == 0){
				SNSManager.GetFaceBookFriends();
				mCnt = 1;
			}else if(mCnt == 1){
				SNSManager.SendInviteMessageEditorTest((result)=>{
					if (result.Error != null){
						Debug.Log(string.Format(" FB Login Error Response:  {0} " + result.Error));
					}
					if (result != null)
					{
