...
public class UIInput : MonoBehaviour
{
	...
	public bool isMultiLine { private get;  set; }
	...
	void OnSelect (bool isSelected)
	{
		...
		if (label != null && enabled && NGUITools.GetActive(gameObject))
		{
			if (isSelected)
			{
				...
				{
#if UNITY_3_4
					mKeyboard = iPhoneKeyboard.Open(mText, (iPhoneKeyboardType)((int)type), autoCorrect,isMultiLine);
#else
					if (isPassword)
					{
					...
					{
						mKeyboard = TouchScreenKeyboard.Open(mText, (TouchScreenKeyboardType)((int)type), autoCorrect,isMultiLine);
