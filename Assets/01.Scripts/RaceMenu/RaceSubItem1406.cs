...
public class RaceSubItem1406 : RaceSubItems {
	...
	public void OnMap1406(){
		if(GV.ChSeasonID >= 6025 && GV.ChSeasonID < 6030){
			var tr= mapInfo.transform.FindChild("Info") as Transform;
			tr.FindChild("BG_On").gameObject.SetActive(true);
			tr.FindChild("lbName").GetComponent<UILabel>().text = Common_Track.Get(1406).Name;
			tr.FindChild("lbMile_1").GetComponent<UILabel>().text = string.Format( KoStorage.GetKorString("73143"), Common_Track.Get(1406).Distance);
			tr.gameObject.SetActive(true);
			tr.FindChild("Flag").FindChild("lbNum").GetComponent<UILabel>().text = string.Format("{0}/{1}",(GV.ChSeasonID - 6025),5);
			tr.FindChild("Effect").gameObject.SetActive(true);
			tr = modeInfo.transform.FindChild("Champion");
			SetChampionInfo(tr);		
		}else if(GV.ChSeasonID >= 6030){
			var tr= mapInfo.transform.FindChild("Info") as Transform;
		//	tr.FindChild("Effect").gameObject.SetActive(true);
			tr.FindChild("lbName").GetComponent<UILabel>().text = Common_Track.Get(1406).Name;
			tr.FindChild("lbMile_1").GetComponent<UILabel>().text = string.Format( KoStorage.GetKorString("73143"), Common_Track.Get(1406).Distance);
