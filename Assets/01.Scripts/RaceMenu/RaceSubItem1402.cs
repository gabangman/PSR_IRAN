...
public class RaceSubItem1402 : RaceSubItems {
	...
	public void OnMap1402(){
		if(GV.ChSeasonID >= 6005 && GV.ChSeasonID < 6010){
			var tr= mapInfo.transform.FindChild("Info") as Transform;
			tr.FindChild("lbName").GetComponent<UILabel>().text = Common_Track.Get(1402).Name;
			tr.FindChild("lbMile_1").GetComponent<UILabel>().text = string.Format( KoStorage.GetKorString("73143"), Common_Track.Get(1402).Distance);
			tr.gameObject.SetActive(true);
			tr.FindChild("Flag").FindChild("lbNum").GetComponent<UILabel>().text = string.Format("{0}/{1}",(GV.ChSeasonID - 6005),5);
		...
		}else if(GV.ChSeasonID < 6005){
			var tr = mapInfo.transform.FindChild("Locked") as Transform;
			tr.gameObject.SetActive(true);
			tr.FindChild("Text1").GetComponent<UILabel>().text = KoStorage.GetKorString("73146");
			bLock =true;
