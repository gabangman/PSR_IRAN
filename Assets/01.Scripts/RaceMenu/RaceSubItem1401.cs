...
public class RaceSubItem1401 : RaceSubItems {
	...
	public void OnMap1401(){
		if(GV.ChSeasonID == 6000) transform.FindChild("Arrow").gameObject.SetActive(true);
		if(GV.ChSeasonID >= 6000 && GV.ChSeasonID < 6005){
			...
			tr.FindChild("lbName").GetComponent<UILabel>().text = Common_Track.Get(1401).Name;
			tr.FindChild("lbMile_1").GetComponent<UILabel>().text = string.Format( KoStorage.GetKorString("73143"), Common_Track.Get(1401).Distance);
			tr.gameObject.SetActive(true);
			tr.FindChild("Flag").FindChild("lbNum").GetComponent<UILabel>().text = string.Format("{0}/{1}",(GV.ChSeasonID - 6000),5);
