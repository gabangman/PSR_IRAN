...
public class RaceSubItem1403 : RaceSubItems {
	...
	public void OnMap1403(){
		if(GV.ChSeasonID >= 6010 && GV.ChSeasonID < 6015){
			var tr= mapInfo.transform.FindChild("Info") as Transform;
			tr.FindChild("BG_On").gameObject.SetActive(true);
			tr.FindChild("lbName").GetComponent<UILabel>().text = Common_Track.Get(1403).Name;
			tr.FindChild("lbMile_1").GetComponent<UILabel>().text = string.Format( KoStorage.GetKorString("73143"), Common_Track.Get(1403).Distance);
			tr.gameObject.SetActive(true);
			tr.FindChild("Flag").FindChild("lbNum").GetComponent<UILabel>().text = string.Format("{0}/{1}",(GV.ChSeasonID - 6010),5);
