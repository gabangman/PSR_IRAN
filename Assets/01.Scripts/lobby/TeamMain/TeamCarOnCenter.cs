...
public class TeamCarOnCenter : MonoBehaviour
{
	/// <summary>
	...
	void OnDragFinished () { if (enabled) RecenterOnCar(); }
	
	/// <summary>
	/// Recenter the draggable list on the center-most child.
	/// </summary>
	
	public void RecenterOnCar ()
	{
		if (mDrag == null)
		{
		...
		if(isResetPos){
			...
			isResetPos = true;
			int centerCar = GV.CarCount;
			Transform t2 = trans.GetChild(centerCar);
		...
		//	Utility.LogWarning("centercar " + centerCar);
		//	Utility.LogWarning("centercar " + t2.position);
		//	Utility.LogWarning("centercar " + offsetCenter);

		//	Utility.LogWarning("centercar " + sqrDist);
		}
