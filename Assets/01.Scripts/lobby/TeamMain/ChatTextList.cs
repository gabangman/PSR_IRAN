...
public class ChatTextList : MonoBehaviour
{
	public enum Style
	{
		Text,
...
//		uiBar.barSize = 1-(float)mParagraphs.Count /(float)maxEntries ;
	...
	//	Utility.LogWarning(" scr " + sb.scrollValue);
	//	float x = sb.scrollValue;
	//	x *= (style == Style.Chat) ? 1f : -1f;
	//	mScroll = Mathf.Max(0f, mScroll + x );
	//	UpdateVisibleText();
	//	mScroll = sb.scrollValue;
	//	UpdateVisibleText();
	}
	...
	{	
		...
		if (mSelected && supportScrollWheel)
		{
			...
			Utility.Log("On 1 " +mScroll);
			UpdateVisibleText();
		}else{
			val *= (style == Style.Chat) ? 10f : -10f;
			mScroll = Mathf.Max(0f, mScroll + val);
			Utility.Log("On 2 " +mScroll);
			UpdateVisibleText();
		}
	}
	...
	void OnChatUp(){
	//	Utility.LogWarning("OnChatUp");
		OnScroll(1);
	}
	
	void OnChatDn(){
	//	Utility.LogWarning("OnChatDn");
		OnScroll(-1);
	}
