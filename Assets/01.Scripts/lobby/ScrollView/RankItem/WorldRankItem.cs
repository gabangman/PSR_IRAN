...
public class WorldRankItem : MonoBehaviour {
	
	
	
	public void ChangeWorldRankContent(int idx){
		return;
		/*
		int listCount = gameRank.instance.listworld.Count;
		if(listCount <= idx){
		...
		gameRank.worldRank wrank = gameRank.instance.listworld[idx];
		...
		userImage = UserDataManager.instance.GetTexture(3000+idx);
		...
		if(Global.gMyWholeRank == 0 || Global.gMyWholeRank > 200){
	...
	IEnumerator fRankPic(int id){
		...
		//	UserDataManager.instance.SaveSetTexture((3000+id),Frank);
