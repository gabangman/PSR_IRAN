...
public class JSONHTTP
{
	...
	protected static Hashtable ParseObject (char[] json, ref int index, ref bool success)
	{
		...
		while (!done) {
			...
			if (token == JSONHTTP.TOKEN_NONE) {
				success = false;
				return null;
			} else if (token == JSONHTTP.TOKEN_COMMA) {
				NextToken (json, ref index);
			} else if (token == JSONHTTP.TOKEN_CURLY_CLOSE) {
				NextToken (json, ref index);
				return table;
			} else {
	...
	protected static ArrayList ParseArray (char[] json, ref int index, ref bool success)
	{
		...
		while (!done) {
			...
			if (token == JSONHTTP.TOKEN_NONE) {
				...
				NextToken (json, ref index);
			} else if (token == JSONHTTP.TOKEN_SQUARED_CLOSE) {
				NextToken (json, ref index);
				break;
			} else {
				object value = ParseValue (json, ref index, ref success);
	...
	protected static object ParseValue (char[] json, ref int index, ref bool success)
	{
		...
		case JSONHTTP.TOKEN_STRING:
			return ParseString (json, ref index, ref success);
		case JSONHTTP.TOKEN_NUMBER:
			return ParseNumber (json, ref index, ref success);
		case JSONHTTP.TOKEN_CURLY_OPEN:
			return ParseObject (json, ref index, ref success);
		case JSONHTTP.TOKEN_SQUARED_OPEN:
			return ParseArray (json, ref index, ref success);
		case JSONHTTP.TOKEN_TRUE:
			NextToken (json, ref index);
			return true;
		case JSONHTTP.TOKEN_FALSE:
			NextToken (json, ref index);
			return false;
		case JSONHTTP.TOKEN_NULL:
			NextToken (json, ref index);
			return null;
		case JSONHTTP.TOKEN_NONE:
			break;
		}
	...
	protected static int NextToken (char[] json, ref int index)
	{
		...
		switch (c) {
		case '{':
			return JSONHTTP.TOKEN_CURLY_OPEN;
		case '}':
			return JSONHTTP.TOKEN_CURLY_CLOSE;
		case '[':
			return JSONHTTP.TOKEN_SQUARED_OPEN;
		case ']':
			return JSONHTTP.TOKEN_SQUARED_CLOSE;
		case ',':
			return JSONHTTP.TOKEN_COMMA;
		case '"':
			return JSONHTTP.TOKEN_STRING;
		case '0':
		case '1':
		case '2':
		case '3':
		...
		case '9':
		case '-':
			return JSONHTTP.TOKEN_NUMBER;
		case ':':
			return JSONHTTP.TOKEN_COLON;
		}
		index--;
