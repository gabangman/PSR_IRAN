...
namespace UnityHTTP
{
    ...
    public class Request
    {
        ...
        public Request( string method, string uri, Hashtable data )
        {
            this.method = method;
            this.uri = new Uri( uri );
			this.byteStream = new MemoryStream(Encoding.UTF8.GetBytes( JSONHTTP.JsonEncode( data ) ));
		...
		{
			List< string > result = new List< string >();
			Dictionary<string,string > mresult = new Dictionary<string, string>();
			foreach (string name in headers.Keys) {
				foreach (string value in headers[name]) {
					//result.Add( name + ": " + value );
					mresult.Add(name,value);
				}
			}
        ...
        private void GetResponse() {
            ...
            try {
                ...
                while (++retry < maximumRetryCount) {
                    ...
                    client.Close ();
					GV.responsStatus = response.status;
				    switch (response.status) {
					case 200: retry = maximumRetryCount; break;
					case 307:
                    case 302:
                    case 301:
		...
		//		Debug.LogException(e);
            ...
            if ( LogAllRequests )
            {
				...
				{
				//	Utility.Log( InfoString( VerboseLogging ) );
					GV.mException =  InfoString( VerboseLogging ) +":"+ response.status.ToString()+".";
				}
				else if ( response != null && response.status >= 400 )
				{
				//	Utility.LogError( InfoString( VerboseLogging ) );
					GV.mException =  InfoString( VerboseLogging ) +":"+ response.status.ToString()+".";
				}
				else
				{
				//	Utility.LogWarning( InfoString( VerboseLogging ) );
					GV.mException =  InfoString( VerboseLogging ) +":"+ response.status.ToString()+".";
				}
        ...
        public virtual void Send( Action< UnityHTTP.Request > callback = null)
        {
            ...
            if ( this.cookieJar != null )
            {
				...
				if(string.IsNullOrEmpty(cookieString) == true){
					cookieString = NetworkManager.instance.wwwCookieString;
				}
		...
		public virtual Dictionary<string,string> wwwSend()
		{
			...
			Dictionary<string,string> mList = GetHeadersDic();
			return mList;
		}
...
#else
            Utility.LogWarning("SSL Cert Error: " + sslPolicyErrors.ToString ());
#endif
            return true;
        }
        ...
        public string InfoString( bool verbose )
        {
			...
			string result = " [ " + method.ToUpper() + " ] [ " + status + " " + message + " ] [ " + sizeString + " ] [ " + responseTime + "ms ]";
